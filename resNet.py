...
class ImageEmbedding(object):
    def __init__(self, hidden_num=128, channels = 3, num_residual_blocks=2):
    ...
    def model(self, images):
        ...
        with tf.variable_scope('conv0', reuse=tf.AUTO_REUSE):
            conv0 = self.conv_bn_relu_layer(images, [3, 3, self.channels, 16], 1)
            layers.append(conv0)
        ...
        with tf.variable_scope('fc', reuse=tf.AUTO_REUSE):
            ...
            output = self.output_layer(global_pool, self.hidden_num)
...
def test_graph(train_dir='logs'):
    ...
    :param train_dir:
    '''
    input_tensor = tf.constant(np.ones([5, 84, 84, 3]), dtype=tf.float32)
    # result = inference(input_tensor, 2, reuse=False)


    image_embed = ImageEmbedding(hidden_num=128, channels=3)
    embedding = image_embed.model(input_tensor)
